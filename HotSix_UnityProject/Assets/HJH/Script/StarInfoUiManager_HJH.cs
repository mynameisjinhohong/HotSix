    ...
    public void SetStarCondition(int stage,int what, int condition, bool clear) //what = 몇번째 게임오브젝트(밑에서 부터 0,1,2) condition = 어떤 조건인지, clear = 이전에 star를 얻었던 것인지 
    {
        ...
        switch (condition)
        {
            ...
            case 1:
                ...
                {
                    text.text = GameManager.instance.starCondition[stage].gameClearTime/60 +":" + GameManager.instance.starCondition[stage].gameClearTime%60 + "초 미만";
                }
                break;
            case 2:
                if (LocalizationSettings.SelectedLocale == LocalizationSettings.AvailableLocales.Locales[0]) //영어
                ...
                {
                    text.text = "메스코인"+GameManager.instance.starCondition[stage].mathCoinAmount + "개 미만 사용";
                }
                break;
        }
